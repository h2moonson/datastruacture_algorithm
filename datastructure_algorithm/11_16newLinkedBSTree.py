...
class LinkedBSTree:
    ...
    def InsertBST(self) -> None:
        ...
        def _insertBST(root, data) -> DNode:
            ...
            return root
        '''
        def _insertBST(root,data): #연습문제 8-4 (삽입: 비재귀적용법)
                pParent=None
                tNode=root
                while tNode: #탐색 실패해야 ok
                    pParent=tNode
                    if data==tNode.data:
                        print("이미 같은 키가 있습니다." )
                        return root
                ...
                if pParent == None: root=DNode(data)
        ...
        while True :
            num = int(input('임의의 정수 입력(종료: 0): '))
            if num == 0 : break
            self.__root = _insertBST(self.__root, num) #삽입 삭제 시 루트노드의 주소값이 바뀐다. 
